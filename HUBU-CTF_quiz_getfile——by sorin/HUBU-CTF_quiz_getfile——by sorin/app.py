...
from flask import Flask, render_template, request, redirect, url_for, session, g, flash, abort, send_file
...
app.config.update(
    ...
    MAX_CONTENT_LENGTH=MAX_ATTACHMENT_SIZE
)
...
def quiz():
    ...
    questions = query_db("SELECT id, title, description, attachment_name FROM questions ORDER BY id")
...
def admin_add():
    ...
    if attachment and attachment.filename != '':
        if attachment.content_length > MAX_ATTACHMENT_SIZE:
            flash("附件大小超过5MB限制")
            return redirect(url_for("admin_dashboard"))
        attachment_data = attachment.read()
        attachment_name = attachment.filename
    
    exec_db("INSERT INTO questions (title, description, answer_hash, points, attachment, attachment_name) VALUES (?,?,?,?,?,?)",
            (title, description, hashlib.sha256(answer.encode()).hexdigest(), int(points), attachment_data, attachment_name))
    flash("已添加题目")
    return redirect(url_for("admin_dashboard"))
...
def admin_update(qid):
    ...
    # 如果既没有新附件也没有删除操作，则保留原有附件
    if attachment_data is None and not remove_attachment:
        current = query_db("SELECT attachment, attachment_name FROM questions WHERE id=?", (qid,), one=True)
        if current:
            attachment_data = current["attachment"]
            attachment_name = current["attachment_name"]
    
    exec_db("UPDATE questions SET title=?, description=?, answer_hash=?, points=?, attachment=?, attachment_name=? WHERE id=?",
            (title, description, hashlib.sha256(answer.encode()).hexdigest(), int(points), attachment_data, attachment_name, qid))
    flash("已更新题目")
    return redirect(url_for("admin_dashboard"))
...
# 附件下载路由
@app.route("/question/attachment/<int:qid>")
@login_required
def download_attachment(qid):
    user = current_user()
    if user["is_admin"]:
        flash("管理员无法下载附件")
        return redirect(url_for("admin_dashboard"))
    
    qrow = query_db("SELECT attachment, attachment_name FROM questions WHERE id=?", (qid,), one=True)
    if not qrow or not qrow["attachment"]:
        abort(404, description="附件不存在")
    
    return send_file(
        BytesIO(qrow["attachment"]),
        as_attachment=True,
        download_name=qrow["attachment_name"]
    )
...
def question_detail(qid):
    ...
    qrow = query_db("SELECT id, title, description, attachment_name FROM questions WHERE id=?", (qid,), one=True)
    if not qrow:
        flash("题目不存在")
        return redirect(url_for("quiz"))
...
def err404(e): return "404 Not Found", 404
