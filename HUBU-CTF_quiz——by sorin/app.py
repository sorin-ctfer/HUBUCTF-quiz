...
def quiz():
    ...
    questions = query_db("SELECT id,text FROM questions ORDER BY id")
...
def submit(qid):
    ...
    qrow = query_db("SELECT text, answer_hash FROM questions WHERE id=?", (qid,), one=True)
    if not qrow:
        flash("题目不存在")
        return redirect(url_for("quiz"))
    ...
    if is_correct:
        ...
        if cnt == 0:
            # first blood, add notification
            exec_db("INSERT INTO notifications (message, created_at) VALUES (?,?)",
                    (f"{user['name']} 获得了题目《{qrow['text']}》的一血！",
    ...
    if is_correct:
        msg = f"回答正确！+{bonus}分" if bonus else "回答正确！"
        if first_rank == 0:
            msg = "（一血！）" + msg
        elif first_rank == 1:
            msg = "（二血！）" + msg
...
def admin_add():
    ...
    if not text or not answer or not points.isdigit():
        flash("信息不完整")
        return redirect(url_for("admin_dashboard"))
    exec_db("INSERT INTO questions (text,answer_hash,points) VALUES (?,?,?)",
            (text, hashlib.sha256(answer.encode()).hexdigest(), int(points)))
    flash("已添加题目")
    return redirect(url_for("admin_dashboard"))
...
def admin_update(qid):
    ...
    if not text or not answer or not points.isdigit():
        flash("信息不完整")
        return redirect(url_for("admin_dashboard"))
    exec_db("UPDATE questions SET text=?, answer_hash=?, points=? WHERE id=?",
            (text, hashlib.sha256(answer.encode()).hexdigest(), int(points), qid))
    flash("已更新题目")
    return redirect(url_for("admin_dashboard"))
